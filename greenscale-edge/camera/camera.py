...
- compute_camera_metrics():
    Capture a frame and compute:
      * avg_color_hex: average color in #RRGGBB (true RGB, not BGR)
...
# Use a single resolution for both metrics and snapshots.
# This is supported on both V2 and V3 and matches your libcamera config.
RESOLUTION = (1280, 720)
...
def _init_camera():
    ...
    if _picam2 is None:
        ...
        # Explicitly request BGR888, which matches what libcamera reports:
        # 1280x720-BGR888/sRGB.
...
def _capture_raw_frame():
    ...
    Returns:
        ...
        (this is what OpenCV expects by default).
    """
    cam = _init_camera()
...
def compute_camera_metrics():
    ...
    """
    # Raw frame is BGR
    frame_bgr = _capture_raw_frame()

    # ---- Average color (RGB) ----
    # Convert to RGB for human-facing color and hex value.
    frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)

    # Downscale for speed; full res isn't needed for an average.
...
def capture_snapshot() -> Path:
    ...
    # Raw frame is already BGR; OpenCV's imwrite expects BGR, so no conversion.
    frame_bgr = _capture_raw_frame()
